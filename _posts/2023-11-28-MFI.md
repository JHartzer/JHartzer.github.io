...
However, this model is only valid if the body frame and IMU frames are equivalent.
\footnote{Having an IMU offset in the body frame is only necessary when multiple IMU are utilized, due to the size constraints of IMU sensors. When only one IMU is used, the origin body frame is typically defined as the origin of the IMU.}
Otherwise, it is necessary to consider the effects of the body motion on the acceleration measured in the IMU frame using the following change of frame formula
